...
const MedicalProfilePage = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    if (file) {
      ...
      try {
        const response = await fetch("http://localhost:5000/upload_medical_report", {
          method: "POST",
          body: formData,
        });
        ...
        if (response.ok) {
          ...
          alert("Medical Report processed! The text has been added to your 'Additional Information' box.");
        } else {
          const errorData = await response.json();
          alert("Error processing report: " + errorData.error);
        }
      } catch (err) {
        console.error("Upload error", err);
        alert("Failed to connect to backend to process the medical report.");
      }
    }
  };
