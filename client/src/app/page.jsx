...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("safeBiteUser");
    if (!token) {
      router.push("/login");
    } else {
      setIsLoggedIn(true);
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center text-white">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-400">Loading SafeBite...</p>
        </div>
      </div>
    );
  }

  // If we reach here and still not logged in (e.g. during redirect), don't render content
    ...
    if (userQuestion.trim()) {
      ...
      try {
        const response = await fetch("http://localhost:5000/talk", {
          ...
          body: JSON.stringify({ question: userQuestion, userProfile: userProfile }),
        });
  ...
  return (
    <div className='min-h-screen bg-gray-900 text-gray-100'>
      ...
      {identifiedFood && (
        ...
        {/* RIGHT COLUMN: AI Chat and Insights */}
        <div className='space-y-8'>
      ...
      <div className='bg-gray-800 shadow-lg rounded-lg overflow-hidden'>
        ...
        <div className='p-4'>
          <button className='w-full border border-blue-500 text-blue-400 hover:bg-blue-900 py-2 rounded-md flex items-center justify-center transition-colors shadow-sm'>
